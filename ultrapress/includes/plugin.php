...
class Ultrapress_Class {
	...
	public static function install_component($args) {
		...
		$url_of_component_dir = $args[ 'url_of_component_dir' ];
	 ...
	 *
 	 * @param string  $key_of_circ  : key of circuit  	 
 	 * @param string  $circuit_path : circuit
	 */
	...
	public static function get_components($key_of_circuit, $is_composed = false) {
		...
		if (! $is_composed	) {
			if (array_key_exists($key_of_circuit , $array_of_circuits)) {
	...
	private function load_dependencies() {
		...
		$dirs = glob( ULTRAPRESS_PATH . 'components/*', GLOB_ONLYDIR);
		...
		// load admin class
		require_once ULTRAPRESS_PATH . 'admin/class-ultrapress-admin.php';
	}
	/** 
